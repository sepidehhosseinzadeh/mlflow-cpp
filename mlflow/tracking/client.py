...
class MlflowClient(object):
    ...
    def log_message(self, run_id, message):
        self.set_tag(run_id, "Message" , "["+str(datetime.datetime.now())+"]: "+message)

    def log_author(self, run_id, author):
        self.set_tag(run_id, "Author" , author)

    def log_docker_id(self, run_id):
        self.set_tag(run_id, "Docker ID", socket.gethostname())

    def log_docker_image_name(self, run_id):
        self.set_tag(run_id, "Docker Image Name", os.environ['FUTURE_CONTAINER_NAME'])
    ...
    def log_artifact(self, run_id, local_path, artifact_path=None):
        ...
        self.set_tag(run_id, "File uploaded", "File: "+local_path+", uploaded into: "+run.info.artifact_uri+" at time: "+str(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")) + ". Access link: " + "https://s3.console.aws.amazon.com/s3/object/" + run.info.artifact_uri[5:] )
    ...
    def log_artifacts(self, run_id, local_dir, artifact_path=None):
        ...
        self.set_tag(run_id, "Dir uploaded", "Dir: "+local_dir+", uploaded into: "+run.info.artifact_uri+" at time: "+str(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")) + ". Access link: " + "https://s3.console.aws.amazon.com/s3/object/" + run.info.artifact_uri[5:] )
